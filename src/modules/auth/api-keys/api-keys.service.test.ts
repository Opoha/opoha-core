...
describe('ApiKeysService', () => {
  ...
  it('creates a hashed key with scoped permissions', async () => {
    ...
    const prisma = {
      permission: {
        findMany: vi.fn().mockResolvedValue([{ id: 'p1', key: 'api-key:read' }]),
      },
      apiKey: { create },
    };
    ...
    expect(create.mock.calls[0][0].data.keyHash).toBe(
  ...
  it('authenticates a valid key and returns scoped permissions', async () => {
    ...
    expect(prisma.apiKey.findUnique).toHaveBeenCalledWith({
      where: { keyHash: hashOpaqueToken('opk_raw') },
      include: {
        user: true,
  ...
  it('rejects revoked API keys', async () => {
    ...
    await expect(service.authenticate('opk_revoked')).rejects.toBeInstanceOf(
