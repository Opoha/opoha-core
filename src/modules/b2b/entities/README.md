...
| `company_price_list_items` | Customer-specific (company ↔ variant) negotiated prices |
| `b2b_quotes` | Buyer quote / PO foundation (`draft`→`submitted`→`accepted`→`converted`) |
...
Cart/order `company_id` columns and B2B approval statuses (`draft` / `approved`) are owned by the **order** module.
