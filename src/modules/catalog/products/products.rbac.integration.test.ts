...
describe('catalog product RBAC (resolver metadata + PermissionsGuard deny)', () => {
  ...
  it('ProductsResolver mutations declare product:* permission keys', () => {
    ...
    expect(
      reflector.get(
        REQUIRE_PERMISSION_KEY,
        ProductsResolver.prototype.products,
      ),
    ).toEqual(['product:read']);
    expect(
      reflector.get(
        REQUIRE_PERMISSION_KEY,
        ProductsResolver.prototype.updateProduct,
      ),
    ).toEqual(['product:update']);
    expect(
      reflector.get(
        REQUIRE_PERMISSION_KEY,
        ProductsResolver.prototype.deleteProduct,
      ),
    ).toEqual(['product:delete']);
  });
